...
df1_spark = df_spark.select('AIRLINE', 'IS_DELAYED').groupBy(['AIRLINE'])\
...
df1.set_index('AIRLINE', inplace=True)
df2 = df1.sort_values('PCT_DELAYS', ascending=False)
#print(df2.head())
df3 = df1.sort_values('TOTAL', ascending=False)
#print(df3.head())
...
df3['TOTAL'].plot.bar(rot=0)
plt.title('Total # flight operations')
plt.ylabel('No. of flight operations per year')
plt.show()
